...
# 500kHz    |   8e6     |   99.915 kHz  

# 1Mhz      |   16e6    |   99.976 kHz  
# 2Mhz      |   32e6    |   99.854 kHz  
# 3Mhz      |   48e6    |   99.976 kHz  
...
class SerialGUI:
    def __init__(self, root: tk.Tk):
        ...
        # Serial
        self.CLK_GEN_F = 80e6
        self.BAUD_MATCH_MULT = 8
        self.BAUDRATE = (CLK_GEN_F / 16) / BAUD_MATCH_MULT
        self.FS = self.CLK_GEN_F / 16
        ...
        # CSV (streaming)
        self.csv_file   = open(f"data_{self.BAUDRATE}.csv", "w", newline="")
    ...
    def _build_sidebar(self, parent):
        ...
        ttk.Label(parent, text=f"{PORT}  ·  {self.BAUDRATE:,} baud",
        ...
        # ── Base Clock Speed Selection ──────────────────────────────────────────
        base_clk_list = [8e6, 16e6, 32e6, 48e6, 64e6, 80e6, 96e6, 112e6, 128e6, 144e6, 160e6]
        base_clk_combo_options = [f"{int(speed / 1e6)} MHz"for speed in base_clk_list]
        

        self.fs_label = ttk.Label(parent, background=CLR["panel"], 
                           foreground=CLR["muted"], font=FONT_LABEL)
        self.fs_label.pack(anchor="w", padx=14, pady=(4, 2))
        self._update_fs_label()  # set initial text

        self.sample_rate_var = tk.StringVar(value="--")
        sample_rate_combo = ttk.Combobox(parent,
            textvariable=self.sample_rate_var,
            values=["--"] + base_clk_combo_options,
        ...
        self.ref_hi_var = tk.StringVar(value="--")
        ...
        ttk.Label(parent, text="LO  (V)",
                  ...
                  font=("Courier New", 9)).pack(anchor="w", padx=14)
        self.ref_lo_var = tk.StringVar(value="--")
        ...
        ttk.Label(parent, text="BATCH CAPTURE",
                background=CLR["panel"], foreground=CLR["muted"],
                font=FONT_LABEL).pack(anchor="w", padx=14, pady=(4, 2))

        self.batch_count_var = tk.StringVar(value="--")
        batch_combo = ttk.Combobox(parent, textvariable=self.batch_count_var,
                                values=["--", "5","10","25","50","100","200"],
                                state="readonly", width=14)
        batch_combo.pack(anchor="w", padx=14, pady=(0, 6))

        batch_btn_row = ttk.Frame(parent, style="Panel.TFrame")
        batch_btn_row.pack(fill="x", padx=14, pady=(0, 4))
        ttk.Button(batch_btn_row, text="▶  Start", style="Accent.TButton",
                command=self.start_batch_capture).pack(side=tk.LEFT, padx=(0, 4))
        ttk.Button(batch_btn_row, text="■  Stop",
                command=self.stop_batch_capture).pack(side=tk.LEFT)
        ...
        # ── Power mode ────────────────────────────────────────────────────────
        ttk.Label(parent, text="POWER MODE",
                  background=CLR["panel"], foreground=CLR["muted"],
                  font=FONT_LABEL).pack(anchor="w", padx=14, pady=(4, 2))

        self.power_mode_var = tk.StringVar(value="--")
        power_mode_combo = ttk.Combobox(parent, textvariable=self.power_mode_var,
                                        values=["--", "Low Power", "Regular", "High Power"],
                                        state="readonly", width=14)
        power_mode_combo.pack(anchor="w", padx=14, pady=(0, 6))
        power_mode_combo.bind("<<ComboboxSelected>>", self.set_power_mode)

        ttk.Separator(parent, orient="horizontal").pack(fill="x", padx=10, pady=8)

        # ── DOUT channel select ───────────────────────────────────────────────
        ttk.Label(parent, text="DOUT CHANNEL",
                  background=CLR["panel"], foreground=CLR["muted"],
                  font=FONT_LABEL).pack(anchor="w", padx=14, pady=(4, 2))

        self.dout_sel_var = tk.StringVar(value="--")
        dout_combo = ttk.Combobox(parent, textvariable=self.dout_sel_var,
                                  values=["--", "TOP", "BOT"],
                                  state="readonly", width=14)
        dout_combo.pack(anchor="w", padx=14, pady=(0, 6))
        dout_combo.bind("<<ComboboxSelected>>", self.set_dout_channel)
    ...
    def update_serial(self):
        if not self.running:
            return
        try:
            if not self._batch_active:          # ← add this guard
    ...
    def start_batch_capture(self):
        try:
            if self.batch_count_var.get() == "--":
                print("Batch: select a capture count first")
                return
            self._batch_total  = int(self.batch_count_var.get())
            self._batch_done   = 0
            self._batch_data   = []   # list of columns, each a list of ints
            self._batch_active = True
            print(f"Batch: starting {self._batch_total} captures...")
            self._batch_trigger_next()
        except Exception as e:
            print(f"Batch error: {e}")

    def _batch_trigger_next(self):
        if not self._batch_active:
            return
        buf_size = int(self.size_var.get())
        self._batch_buf_size    = buf_size
        self._batch_capture_buf = []
        self.ser.reset_input_buffer()          # flush any stale bytes first
        self.ser.write(bytes([0x02, 0x00, 0x00]))
        print(f"Batch: triggering {self._batch_done + 1}/{self._batch_total}...")
        self.root.after(1, self._batch_collect)
    ...
    def _batch_collect(self):
        ...
        try:
            ...
            self._batch_capture_buf.extend(chunk)
        except Exception as e:
            print(f"Batch collect error: {e}")
            self._batch_active = False
            return

        if len(self._batch_capture_buf) < self._batch_buf_size:
            self.root.after(1, self._batch_collect)
            return

        self._batch_data.append(self._batch_capture_buf[:self._batch_buf_size])
        self._batch_done += 1
        print(f"Batch: captured {self._batch_done}/{self._batch_total}")

        # update plot so you can see each capture come in
        self.buffer = deque(self._batch_capture_buf[:self._batch_buf_size],
                            maxlen=self._batch_buf_size)
        self.line.set_data(self.x, self.buffer)
        self.dots.set_data(self.x, self.buffer)
        self.canvas.draw_idle()
    ...
    def _batch_save(self):
        ...
        # ── Folder level 1: DOUT channel (TOP / BOTTOM) ──────────────────────
        dout_raw = self.dout_sel_var.get()
        dout_folder = {"TOP": "TOP", "BOT": "BOTTOM"}.get(dout_raw, dout_raw)

        # ── Folder level 2: Power mode (HPM / LPM / RPM) ─────────────────────
        pwr_raw = self.power_mode_var.get()
        pwr_folder = {
            "High Power": "HPM",
            "Low Power":  "LPM",
            "Regular":    "RPM",
        }.get(pwr_raw, pwr_raw.replace(" ", "_").lower())

        # ── Folder level 3: Sampling rate (500k, 1M … 10M) ───────────────────
        fs_hz = self.FS          # e.g. 1_000_000 for 1 MHz
        if fs_hz >= 1e6:
            fs_folder = f"{int(round(fs_hz / 1e6))}M"
        else:
            fs_folder = f"{int(round(fs_hz / 1e3))}k"
        ...
        save_dir = os.path.join(dout_folder, pwr_folder, fs_folder)
        os.makedirs(save_dir, exist_ok=True)

        filename = f"batch_{n}x{buf}_hi{hi}V_lo{lo}V.csv"
        ...
        try:
            with open(filepath, "w", newline="") as f:
                writer = csv.writer(f)
                # header row
                writer.writerow([f"test_{i+1}" for i in range(self._batch_total)])
                # rows = sample index across all columns
                for row_i in range(self._batch_buf_size):
                    writer.writerow([self._batch_data[col][row_i]
                                    for col in range(self._batch_total)])
            print(f"Batch: saved {self._batch_total} captures → {filepath}")
        except Exception as e:
    ...
    def stop_batch_capture(self):
        if getattr(self, "_batch_active", False):
            self._batch_active = False
            print(f"Batch: stopped after {self._batch_done}/{self._batch_total}")

    # ── Power Mode ──────────────────────────────────────────────────────────

    def set_dout_channel(self, _event=None):
        sel = self.dout_sel_var.get()
        commands = {
            "TOP": bytes([0x0A, 0x00, 0x00]),
            "BOT": bytes([0x0A, 0x00, 0x01]),
        }
        if sel in commands:
            self.ser.write(commands[sel])
            print(f"DOUT channel → {sel} ({' '.join(f'{b:02X}' for b in commands[sel])})")
    ...
    def change_clock_speed(self, _event=None):
        try:
            ...
            self.CLK_GEN_F   = int(selected.split(" MHz")[0]) * 1e6
            self.FS          = self.CLK_GEN_F / 16
            self.BAUDRATE    = self.FS / self.BAUD_MATCH_MULT
        ...
        # Reconfigure the serial port
            ...
            print(f"Sampling rate → CLK_GEN_F={self.CLK_GEN_F} |  BAUD_RATE = {self.BAUDRATE} | SAMPLING_RATE = {self.FS}")
        except Exception as e:
    ...
    def _update_fs_label(self):
        fs = self.FS
        if fs >= 1e6:
            text = f"SAMPLING RATE = {fs/1e6:.3g}MHz"
        elif fs >= 1e3:
            text = f"SAMPLING RATE = {fs/1e3:.3g}kHz"
        else:
            text = f"SAMPLING RATE = {fs}Hz"
        self.fs_label.config(text=text)
    ...
    def set_power_mode(self, _event=None):
        mode = self.power_mode_var.get()
        commands = {
            "Low Power":  bytes([0x07, 0x00, 0x22]),
            "Regular":    bytes([0x07, 0x00, 0x48]),
            "High Power": bytes([0x07, 0x00, 0xFF]),
        }
        if mode in commands:
            self.ser.write(commands[mode])
            print(f"Power mode → {mode} ({' '.join(f'{b:02X}' for b in commands[mode])})")
    ...
    def _default_filename(self) -> str:
        ...
        hi  = self.ref_hi_var.get()
        lo  = self.ref_lo_var.get()
        pwr = self.power_mode_var.get().replace(" ", "_").lower()
        return f"buf{buf}_ce{ce}_hi{hi}V_lo{lo}V_{pwr}.csv"
