...
class SerialGUI:
    ...
    def _batch_save(self):
        fs_str  = self.fs_label.cget("text").replace("SAMPLING RATE = ", "").replace(" ", "")
        buf     = self.size_var.get()
        hi      = self.ref_hi_var.get()
        lo      = self.ref_lo_var.get()
        n       = self._batch_total
        pwr     = self.power_mode_var.get().replace(" ", "_").lower()
        filename = f"batch_{n}x{buf}_{fs_str}_hi{hi}V_lo{lo}V_{pwr}.csv"
        ...
        try:
            ...
            print(f"Batch: saved {self._batch_total} captures → {filename}")
        except Exception as e:
