...
module top_v4(
    ...
    clk_wiz_base base (
        ...
        .clk_out1 (clk_100M),
        .locked   (clk_locked)
        // Note: clk_wiz_base has no reset input needed here,
        // it locks on its own from sysclk
    );
    ...
    // Using clk_100M here since that's our first stable clock
    wire btn0_db, btn1_db;
    wire btn0_pulse, btn1_pulse;
    reg  btn0_db_d, btn1_db_d;
    ...
    // --- Final reset and resetn ---
    wire reset  = btn0_pulse | por_reset;
    wire resetn = ~reset;
    ...
    wire clk_300M;
    wire clk_sel_locked;
    reg [7:0] clk_selection;
    
    clk_selector u_clk_sel (
        .clk_100mhz (clk_100M),
        .resetn     (resetn),
        .freq_sel   (clk_selection),   // driven by UART command 8'h01
        .clk_out    (clk_300M),
        .locked     (clk_sel_locked)
    );
    
    // Everything downstream stays in reset until both clocks are locked
    // Use this anywhere you currently use reset if that logic runs on clk_300M:
       wire reset = btn0_pulse | por_reset | ~clk_sel_locked;
    // For now reset is already defined above - add ~clk_sel_locked if needed.
